...
perc_used <- perc_used %>% group_by(used) %>% summarize(hpounds = sum(hpounds)) %>% 
  mutate(tot = sum(hpounds), perc = hpounds / tot * 100)
...
mb1 <- sampled_rums(data_in = tows_clust, the_port = "ASTORIA / WARRENTON", 
  min_year = 2007, max_year = 2009,
  risk_coefficient = 1, ndays = 30, focus_year = 2009, 
  nhauls_sampled = 50, seed = 7, ncores = 6, rev_scale = 100, model_type = "no_bycatch")
...
#Work out the sampling algorithm

tows_clust %>% distinct(haul_id, .keep_all = T) %>% filter(depth_bin != 69) %>% 
  group_by(set_year, depth_bin) %>% summarize(nvals = length(unique(haul_id))) %>%
  ggplot() + geom_bar(aes(x = depth_bin, y = nvals), stat = 'identity') + 
  facet_wrap(~ set_year, ncol = 4) + coord_flip() + scale_x_reverse()



  
   %>% dcast(depth_bin ~ set_year, value.var = 'nvals')
